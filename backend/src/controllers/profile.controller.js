...
const updateProfile = async (req, res) => {
  try {
    const userId = parseInt(req.params.id);
    const { username, pronouns, allergens, age, height, weight, targetWeight, trackingWeeks, activityLevel, email, photoUrl, muscleGroups,} = req.body;

    const updatedUser = await prisma.user.update({
      where: { id: userId },
      data: { username, pronouns, allergies: allergens, email: email, photoUrl: photoUrl, muscleGroups: muscleGroups}
    });
